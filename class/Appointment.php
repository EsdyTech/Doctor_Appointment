...
class Appointment
{
	public $base_url = 'http://localhost:8083/websites/doctor-appointment/';
	public $connect;
	public $query;
	public $statement;
	public $now;
	...
	function get_doctor_total_today_appointment()
	{
		$this->query = "
		SELECT * FROM appointment_table 
		INNER JOIN doctor_schedule_table 
		ON doctor_schedule_table.doctor_schedule_id = appointment_table.doctor_schedule_id 
		WHERE doctor_schedule_date = CURDATE() AND appointment_table.doctor_id = '".$_SESSION["admin_id"]."'";

		$this->execute();
		return $this->row_count();
	}

	function get_doctor_total_yesterday_appointment()
	{
		$this->query = "
		SELECT * FROM appointment_table 
		INNER JOIN doctor_schedule_table 
		ON doctor_schedule_table.doctor_schedule_id = appointment_table.doctor_schedule_id 
		WHERE doctor_schedule_date = CURDATE() - 1 AND appointment_table.doctor_id = '".$_SESSION["admin_id"]."'";
		$this->execute();
		return $this->row_count();
	}

	function get_doctor_total_seven_day_appointment()
	{
		$this->query = "
		SELECT * FROM appointment_table 
		INNER JOIN doctor_schedule_table 
		ON doctor_schedule_table.doctor_schedule_id = appointment_table.doctor_schedule_id 
		WHERE doctor_schedule_date >= DATE(NOW()) - INTERVAL 7 DAY AND appointment_table.doctor_id = '".$_SESSION["admin_id"]."'";

		$this->execute();
		return $this->row_count();
	}

	function get_doctor_total_appointment()
	{
		$this->query = "
		SELECT * FROM appointment_table WHERE doctor_id = '".$_SESSION["admin_id"]."'";
